...
#define NOTIFY_FADE_IN_OUT_TIME			500		// Fade in and out duration
...
public:
	...
	NOTIFY_INLINE auto get_elapsed_time() { return 1000* (ImGui::GetTime() - this->creation_time); }
...
public:
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->creation_time = ImGui::GetTime();
...
namespace ImGui
{
	...
	NOTIFY_INLINE void RemoveNotification(size_t index)
	{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (size_t i = 0; i < notifications.size(); i++)
		{
			...
			PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.0f, 78 / 255.0f, 157 / 255.0f, 1.0f));
