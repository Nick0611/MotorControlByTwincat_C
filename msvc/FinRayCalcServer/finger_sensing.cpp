...
Grad_Obj_return finger_sensing(VectorXd q, VectorXd Ft, VectorXd fr, double fc, double lc, parameters par, MatrixXd Seq, Vector4d ds0)
{
	...
	for (int ii = 0; ii < num; ++ii)
	{
		...
		xi = x.col(ii);
		Jt.col(ii) = At * xi;
		rxi.col(ii) = wed(e3) * Jt.block(3, ii, 3, 1);
		gi = expm(wed(xi), qi);
		Ai = Ad(gi);
		gst = gst * gi;
		At = At * Ai;
	...
	VectorXd Wc(6), Fc(6), dFc_l(num), dFc_f(num);
	...
	for (int kk = 0; kk < MM; ++kk)
	{
		...
		rib.col(2 * kk) << rSk.topRows(2);
		rib.col(2 * kk + 1) << rBk.topRows(2);
		frk = fr(kk);
		lrk = (rSk - rBk).norm();
		urk = (rSk - rBk) / lrk;
...
//#pragma omp for schedule(static,int(num/8))
	...
	Kt = KJt;
	//
	VectorXd dg(6), dl(5), ds(4), dt(num), dY(num + 15);
	MatrixXd dt_dq(num, num), Jac(num + 15, num + 13);
	dg = Vee(logm(gst));
	dl = lr - lr0;
	ds = Seq*q - ds0;
	...
	Jac.block(11, 0, num, num) = dt_dq;
	Jac.block(11, num, num, 6) = -Jt.transpose();
	Jac.block(11, num + 6, num, 5) = -dt_df;
	Jac.block(11, num + 11, num, 1) = -dFc_l;
	Jac.block(11, num + 12, num, 1) = -dFc_f;
	Jac.block(num + 11, 0, 4, num) = Seq;
	Grad_Obj_return gor;
	gor.c = dY;
	gor.gradient = Jac;
	gor.rib = rib;
